...
begin
  ...
  PROCESS (state)
   BEGIN
			CASE state IS
			  WHEN s0 => if (dir = '0') then
					phases <= "0001";
				 elsif (dir = '1') then
					phases <= "1000";
				 end if;
			  WHEN s1 => if (dir = '0') then
					phases <= "0010";
				 elsif (dir = '1') then
					phases <= "0100";
				 end if;
			  WHEN s2 => if (dir = '0') then
					phases <= "0100";
				 elsif (dir = '1') then
					phases <= "0010";
				 end if;
			  when s3 => if (dir = '0') then
					phases <= "1000";
				 elsif (dir = '1') then
					phases <= "0001";
				 end if;
  ...
  topCounter <= 10000000 when vel = "00" else
								...
								500000 when vel = "00" else
                  100000;

  process(clk)
	variable counter : integer range 0 to 50000000 := 0;
	variable max_step : integer := 0;
  begin
    if (rising_edge(clk)) then
